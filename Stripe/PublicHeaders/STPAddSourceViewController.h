...
/** You can use this view controller to collect information for reusable payment sources. It supports two types: `STPSourceTypeCard` and `STPSourceTypeSEPADebit`. On submission, it will use the Stripe API to convert the information entered by the user into a Stripe Source object. It renders a right bar button item that submits the form, so it must be shown inside a `UINavigationController`.
 */
@interface STPAddSourceViewController : STPCoreTableViewController

/**
 *  Initializes a new `STPAddSourceViewController` for collecting information for the given source type.
 *  If the given source type is unsupported, this initializer will return nil.
 *  Don't forget to set the `delegate` property after initialization.
 *
